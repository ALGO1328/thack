...
def generateToken():
    ...
    print(jwt.decode(token, API_SEC, algorithms=["HS256"]))
...
def createMeeting(creator, time1):
    headers = {'authorization': f'Bearer  + {generateToken()['iss']}',
               'content-type': 'application/json'}
    r = requests.post(
        f'https://api.zoom.us/v2/users/me/meetings',
        headers=headers, data=json.dumps(meetingdetails(creator, time1)))
...
print(createMeeting('@algo1328', 1714244140))
